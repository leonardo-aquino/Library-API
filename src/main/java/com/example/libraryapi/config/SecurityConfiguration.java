...
@EnableMethodSecurity(securedEnabled = true ,jsr250Enabled = true) // anotação para adicionarmos nos controles quais Roles podem acessar determinados métodos
...
//     O parâmetro HttpSecurity permite configurar autenticação, autorização, CSRF, login, etc.
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception{
        return http
                .csrf(AbstractHttpConfigurer::disable) //CSRF é uma proteção usada em aplicações web, Aqui está sendo desabilitada, geralmente porque não precisamos dela em API REST
                ...
                .authorizeHttpRequests( authoriza -> {
                    authoriza.requestMatchers(HttpMethod.POST,"/usuarios").permitAll();
